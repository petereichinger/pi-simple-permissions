...
- Agent bash commands require confirmation.
- User `!` / `!!` bash commands also require confirmation by default.
...
When a bash command is requested, the dialog lets you:
...
- Pi does not currently have a separate built-in delete-file tool. Deletes usually happen through `bash` (`rm`, etc.), so they are covered by the bash confirmation path rather than path-specific delete analysis.
